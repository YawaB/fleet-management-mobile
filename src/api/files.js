...
export function reformatNetGeoData(data) {
  ...
  let index = data.findIndex(
    (r) => r.__EMPTY == "Distance" || r.__EMPTY_1 == "Longueur"
  );
...
export function reformatPlanCaseetteData(data, options) {
  ...
  let output = data.reduce((c, val) => {
    ...
    if (val.sheet == "Liste des cassettes")
      ...
      ["boxPlanVerifier"].includes(options.type) &&
      val.sheet.includes("Détails du contenant")
    )
      filter = ["Nom", "Capacité", "Nb de tubes"];

    if (filter) {
      let result = (c[val.sheet] = reformatDataWithCriteria(
        val.data,
        filter,
        options?.keys?.[val.sheet]
      ));
...
function reformatDataWithCriteria(data, matchKeys, includeKeys) {
  ...
  for (let [key, val] of Object.entries(headers)) {
    ...
    if (headers_group[val]?.length > 1)
      headers[key] = val + "." + (headers_group[val]?.length - 1);
  }
  ...
  for (const dt of data) {
    ...
    for (const key of keys) {
      ...
      if (isArray) {
        if (!includeKeys.includes(headers[key].split(".")[0]))
