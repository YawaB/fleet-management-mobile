...
import { getCurrentUser, logout } from '../modules/Authentication/slice/auth.slice';
import { getUiParams } from '../store/slice/ui';

function MenuComponent({navigator}) {
  ...
  return (
      <View className="flex-row p-2 bg-orange-500 justify-between items-center shadow-3">
        <Ionicons name='arrow-undo-outline' color={'#fff'} size={30} onPress={goBack}/>
        <View>
          <Text clasName="text-white font-bold" style={{color:'#fff' , fontWeight: 'bold'}}>
            {uiParams?.title}
          </Text>
        </View>
        <View  clasName="flex-row  items-center justify-center bg-white">
            <Menu
                ...
                anchor={
                  current_user ? (
                    <TouchableOpacity  onPress={openMenu}  className="h-8 w-8 items-center justify-center rounded-full bg-white">
                        <Text>{current_user?.pseudo}</Text>
                    </TouchableOpacity>
                    )  : <Ionicons size={20} onPress={openMenu} name='ellipsis-vertical-outline' />
                    }
              >
                    ...
                    <Divider />
                     <Menu.Item  onPress={()=> navigateTo('Features')} title="Applications" />
                    <Divider />
