...
export function formatDate(date, options = {format:"DD/MM/YYYY HH:mm" }) {
    let defaultOptions = {format:"DD/MM/YYYY HH:mm"}
    ...
    let outputDate = options.format

    for(let [key , val] of Object.entries(dateObj) ){
        outputDate =  outputDate.replace(key, val)
    }
...
export async function printHtml(html){
    ...
    if(typeof uri == 'string'){
        Sharing.shareAsync(uri).then(response =>{
