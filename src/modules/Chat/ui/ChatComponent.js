...
function ChatComponent({ obj, onBack }) {
  ...
  const toggleSelect = (_message, remove) => {
    ...
    if (selectedChats.includes(_message.id) || remove)
      setSelectedChats((prev) => prev.filter((o) => o != _message.id));
  ...
  return (
    <View style={{ flex: 1 }}>
      <View
        className="items-center justify-around p-2 flex-row"
        ...
        <View className="">
          {selectedChats?.length > 0 && (
            <View className="flex-row gap-5">
              ...
              {selectedChats.length == 1 && (
                <Ionicons
                  onPress={(e) => setReplyTo(selectedChats[0])}
                  name="arrow-undo-outline"
                  color={"#fff"}
                  size={20}
                />
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            className={`w-full rounded-md my-1 p-2 ${
              selectedChats.includes(item.id) ? "bg-gray-100" : ""
            } ${item?.from == user?.id ? "items-end" : ""}`}
          >
            <View style={{ width: "70%" }} className="p-2 bg-white rounded-lg">
              <Text className="font-semibold text-blue-600">
                {item?.from == user?.id ? "Vous" : item?.sender?.fullname}
              </Text>
              <View className="mt-2">
              ...
              <View className="items-end">
                <Text className="text-gray-400">
                  {moment(item.date || item.createdAt).format("HH:mm")}
                </Text>
              </View>
            </View>
          </TouchableOpacity>
