...
export const createOrUpdateSite = createAsyncThunk(
  ...
  async (_args, { dispatch, getState }) => {
    try {
      ...
      if (!data.id) res = await _createSite(data);
      else res = await _updateSite(data);
...
export const { setSites, setSelectedSite, setEditSite, setSiteView } =
  siteslice.actions;
